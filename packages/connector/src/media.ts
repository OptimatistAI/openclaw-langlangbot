...
export type RegisterOutboundAttachmentInput = {
  ...
  filename: string;
  /**
   * Omit to let the sidecar sniff it. A supplied value wins outright, so a
   * caller guessing from a smaller table than the sidecar's can only downgrade
   * the resulting `kind`.
   */
  mime?: string;
  sha256?: string;
...
export type RegisterOutboundAttachmentResult = {
  ...
  status: AttachmentStatus;
  download_url: string;
  /** What was actually stored — build content parts from these, not guesses. */
  mime: string;
  kind: AttachmentKind;
  size: number;
};
