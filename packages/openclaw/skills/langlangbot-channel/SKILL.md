...
openclaw cron add \
  ...
  --at 2m \
  --session isolated \
  --session-key 'agent:main:langlangbot:default:direct:conversation:<uuid>' \
...
This channel uses the gateway default agent id (usually `main`; override with `channels.langlangbot.agentId`). Two upstream OpenClaw behaviors affect Operator **allow-always** and multi-line exec output (e.g. `ss -lntp`). Workaround: add shared allowlist entries under `agents["*"]` in `~/.openclaw/exec-approvals.json`. Full write-up: repo `docs/OPENCLAW-EXEC-KNOWN-ISSUES.md`.
